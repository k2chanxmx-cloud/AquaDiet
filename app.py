...
【トレーニング記録の読み方】
・exercises の amount は、その種目の正式な回数または時間。
・unit が「回」なら「腹筋60回」のように、amount と unit をセットで読む。
・unit が「分」なら「ランニング30分」のように、amount と unit をセットで読む。
・amount が記録されている場合、memo より amount と unit を優先する。
・memo は補足情報として扱い、正式な回数/時間の代わりにしない。
・memo に回数が書かれていても、amount が存在するなら amount を正式な記録として扱う。
・トレーニング記録がある日は、コメント内で最低1回は「種目名＋回数/時間」を具体的に言う。
・複数種目ある場合、可能な範囲でそれぞれの回数/時間に触れる。
